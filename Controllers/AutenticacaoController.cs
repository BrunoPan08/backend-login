...
namespace ProjetoKeener3.Controllers
{
    ...
    {
        private readonly UserManager<AplicacaoUsuario> userManager;
        private readonly RoleManager<IdentityRole> roleManager;
        private readonly IConfiguration _configuration;

        public AutenticacaoController(UserManager<AplicacaoUsuario> userManager, RoleManager<IdentityRole> roleManager, IConfiguration configuration)
        {
        ...
        {
            var user = await userManager.FindByNameAsync(model.Nome);
            if (user != null && await userManager.CheckPasswordAsync(user, model.Senha))
            {
        ...
        public async Task<IActionResult> RegisterAdmin([FromBody] RegistroModel model)
        {
            var userExists = await userManager.FindByNameAsync(model.Nome);
            if (userExists != null)
                return StatusCode(StatusCodes.Status500InternalServerError, new Response { Status = "Error", Message = "Usuário já existe!" });
